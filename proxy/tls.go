...
func tlsChannel(conn net.Conn, bufR *bufio.Reader, cSess *session.ConnSession, resp *http.Response) {
	defer func() {
	...
	for {
		...
		pl := proto.GetPayloadBuffer()          // 从池子申请一块内存，存放去除头部的数据包到 PayloadIn，在 payloadInToTun 中释放
		...
		case 0x04:
			log.Debug("tls receive DPD-RESP")
		case 0x00: // DATA
